...
/* serial event */
static osEventFlagsId_t event_serial = NULL;
...
static void serial_soft_trans_irq(void *parameter)
{
    ...
    {
        MODBUS_DEBUG("Master create event_serial success!\r\n");
    }
    else
    {
    ...
    while (1)
    {
        ...
        osEventFlagsWait(event_serial,                    /* 事件对象句柄 */
                         EVENT_SERIAL_TRANS_START,        /* 接收任务感兴趣的事件 */
                         osFlagsWaitAny | osFlagsNoClear, /* 退出时清除事件位 */
                                                          /* 满足感兴趣的所有事件 */
